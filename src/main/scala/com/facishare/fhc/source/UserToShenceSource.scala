  ...
  def getUserInfoDF(hiveContext: HiveContext,runDate:String):RDD[(String,JMap[String,Object])]={
    ...
    val sql=
      """
        ...
        left join
        (select C.enterprise_id,D.dept_name from
          (select distinct enterprise_id,seller_dept_id from dw_bds_m.m_pub_enterprise_partner where dt='%s' and seller_dept_id is not null) C
          left Join
          (select department_id,dept_name from dw_bds_m.m_pub_org_hj_department where dt='%s' and department_id is not null) D  on  C.seller_dept_id=D.department_id

...
object CommonSQLSource {
       ...
       val commonRDD:RDD[Tuple3[String, String, JMap[String, Object]]]=commonDF.map(row=>{
         ...
         for(index <- fieldsArray.indices){
           ...
           fieldsArray(index).dataType.typeName match {
             ...
             case "int"| "integer" | "long" | "double" | "float" | "short" =>{
  ...
  /**
    ...
    * @return
    */
  def createRecordTuple(dataFrame: DataFrame, eventName:String, distinctIDName:String):RDD[Tuple3[String, String, JMap[String, Object]]]={
    val structType=dataFrame.schema
    val fieldsArray=structType.fields
    //postgresql 列名默认小写
    val isContains=fieldsArray.map(field=>{field.name}).contains(distinctIDName.toLowerCase)
    if(!isContains){
      throw new RuntimeException("sql fields does not contains distinctIDName:'"+distinctIDName.toLowerCase+"'")
    }
    val commonRDD:RDD[Tuple3[String, String, JMap[String, Object]]]=dataFrame.map(row=>{
      val map = new util.HashMap[String, Object]()
      var distinctID="-10000"
      for(index <- fieldsArray.indices){
        var value:Object=null
